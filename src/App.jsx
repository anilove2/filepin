...
function PublicPage({ username, filename, onBack, dark, onToggle }) {
  ...
  async function tryPin() {
    if (!file) return;
    // Block access if already expired or downloaded
    ...
    // Generate signed URL with generous window for slow connections
    const { data } = await supabase.storage.from("files").createSignedUrl(file.storage_path, 3600);
    setDlUrl(data?.signedUrl || null);
    // Increment download count — this is what gates future access for 1_download files
    await supabase.rpc("increment_downloads", { file_id: file.id });
    setStage("granted");
    // Storage cleanup is handled server-side by Supabase cron — not here
  }
          ...
          <>
            ...
            {stage==="expired" ? (
              <div style={{padding:"24px 0",textAlign:"center"}}>
                ...
                <p style={{color:t.textSub,fontSize:13,fontFamily:"monospace",lineHeight:1.6}}>This was a single-use link. The file has already been downloaded and is no longer accessible.</p>
              </div>
            ) : stage==="granted" ? (
              <div style={{padding:"20px 0"}}>
                ...
                {file.expiry_type==="1_download" && (
                  <p style={{color:t.textMuted,fontSize:11,fontFamily:"monospace",marginTop:10,lineHeight:1.6}}>
                    ⚠ Single-use link — this file will not be accessible again after download
                  </p>
                )}
              </div>
            ) : (
